...
const CSA_TOOLS = [
  ...
  },
  {
    ...
    description: 'Generate TEXT-FREE background visuals only (no headline, no CTA). IMPORTANT: For complete ad creatives WITH text, use generate_ad_creative_with_text instead — it is the primary tool. Only use this tool when the user explicitly asks for a raw visual/background without any text overlay. Images are generated via DALL-E 3, Flux Pro, or Imagen 3 with smart fallback.',
  ...
  // --- Template Overlay Creative ---
  {
    ...
    description: 'PRIMARY TOOL FOR AD CREATIVES — Generate agency-quality ad creatives with professional text overlay. This is the DEFAULT tool for any ad/creative request. Auto-researches the client\'s website for brand context and searches competitor ads from Meta Ad Library before generating. Pipeline: (1) Auto-scrape website + competitor ads for context, (2) Generate dramatic AI background with brand colors and product focus, (3) Generate ad copy (headline + subtext + CTA) matched to brand voice, (4) Render professional text overlay with Montserrat/Inter fonts, brand-colored CTA, and modern layout via Puppeteer. Returns both Feed (1080x1080) and Stories (1080x1920) formats. ALWAYS use this instead of generate_ad_images for any ad request.',
    input_schema: { type: 'object', properties: { clientName: { type: 'string', description: 'Client name' }, platform: { type: 'string', enum: ['meta', 'instagram', 'google', 'tiktok'], description: 'Target platform' }, product: { type: 'string', description: 'Product or service being advertised' }, goal: { type: 'string', description: 'Campaign goal: awareness, conversion, promotion, leads' }, concept: { type: 'string', description: 'Visual concept for the background image — be specific about the scene, product, and mood' }, mood: { type: 'string', description: 'Mood/emotion: premium, energetic, urgent, calm, aspirational, bold' }, style: { type: 'string', description: 'Creative style: cinematic, product-hero, lifestyle, editorial, minimalist, dramatic, luxury' } }, required: ['clientName', 'platform'] },
  },
  // --- Diagnostics ---
  {
