...
const CSA_TOOLS = [
  ...
  // --- Report Export ---
  {
    ...
    input_schema: { type: 'object', properties: { clientName: { type: 'string', description: 'Client or brand name for the report header' }, type: { type: 'string', enum: ['social_strategy', 'content_calendar', 'competitor_analysis', 'monthly_report', 'custom'], description: 'Report type. content_calendar renders as a visual weekly grid. custom allows freeform instructions.' }, data: { type: 'string', description: 'Report data — can be structured JSON or natural language description of what to include. For content calendars: include month, platforms, posts per week, themes. For strategies: include goals, audience, content pillars. For competitor analysis: include competitor names and insights.' }, imageUrls: { type: 'string', description: 'Comma-separated image URLs to embed in the report. Pass creative preview URLs, competitor ad screenshots, site screenshots, or any visual assets. These get rendered as <img> tags in the PDF alongside the corresponding content.' }, customPrompt: { type: 'string', description: 'Custom instructions for the report design/content (used with type "custom")' } }, required: ['clientName', 'type', 'data'] },
  },
  {
  ...
  {
    name: 'edit_image',
    description: 'Edit an existing image using natural language instructions while preserving the person\'s identity and likeness. Uses Flux Kontext AI. Perfect for: changing outfits ("put me in a professional suit"), swapping backgrounds ("make background a modern office"), adjusting style/mood, removing objects. ALWAYS use this instead of generate_ad_images when the user uploaded a photo and wants MODIFICATIONS to it. Cost: ~$0.04/edit.',
    input_schema: { type: 'object', properties: { imageUrl: { type: 'string', description: 'URL of the image to edit (use the uploaded photo URL from conversation)' }, prompt: { type: 'string', description: 'Edit instruction in natural language (e.g. "change the shirt to a professional navy suit and tie", "make the background a sleek modern office", "remove the background completely")' }, model: { type: 'string', enum: ['flux-kontext', 'nano-banana-pro'], description: 'flux-kontext (default): precise edits preserving identity. nano-banana-pro: broader style changes and background swaps.' }, format: { type: 'string', description: 'Output size: meta_feed, meta_square, meta_story, instagram_feed, instagram_story, general (default)' }, clientName: { type: 'string', description: 'Client name for cost tracking (optional)' } }, required: ['imageUrl', 'prompt'] },
  },
  {
